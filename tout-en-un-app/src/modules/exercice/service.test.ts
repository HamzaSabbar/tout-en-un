...
import {
  ...
  listerExercices,
  obtenirAideExercice,
  obtenirCorrectionExercice,
  obtenirCorrectionVideoExercice,
  obtenirEnoncesExercices,
...
describe("obtenirEnoncesExercices", () => {
  it("exige un exercice publié, groupe tous les énoncés d'un cours en une requête", async () => {
    findManyExercice.mockResolvedValue([
      { id: BigInt(9), enonce: JSON.parse(ENONCE) },
      { id: BigInt(10), enonce: null },
    ]);

    const enonces = await obtenirEnoncesExercices(BigInt(1), BigInt(3));
    ...
    expect(requete.select).toEqual({ id: true, enonce: true });
    expect(enonces.get("9")).not.toBeNull();
    expect(enonces.get("10")).toBeNull();
  });
});

// Le cœur du parcours à étapes : ce qui n'est pas encore dû à l'élève ne quitte
// pas le serveur. C'est l'appelant (les routes `/aide` et `/correction`) qui
// vérifie le journal avant d'appeler ces fonctions ; elles ne vérifient que la
// visibilité de l'exercice.
describe("obtenirAideExercice", () => {
  it("rend l'aide d'un exercice publié", async () => {
    findFirstExercice.mockResolvedValue({
      aide: JSON.parse(JSON.stringify({ version: 1, noeuds: [{ type: "paragraphe", texte: "Pense à $g$." }] })),
    });
    const aide = await obtenirAideExercice(BigInt(1), BigInt(9));
    expect(aide).not.toBeNull();
  });

  it("rend null quand l'exercice n'a pas d'aide", async () => {
    findFirstExercice.mockResolvedValue({ aide: null });
    expect(await obtenirAideExercice(BigInt(1), BigInt(9))).toBeNull();
  });
...
describe("obtenirCorrectionExercice", () => {
  it("rend la correction et annonce l'existence d'une vidéo, sans sa référence", async () => {
    findFirstExercice.mockResolvedValue({
    ...
    const correction = await obtenirCorrectionExercice(BigInt(1), BigInt(9));

    expect(correction?.texte).not.toBeNull();
    expect(correction?.videoDisponible).toBe(true);
    // La référence de la vidéo n'est jamais rendue avec la correction : elle
    // passe par sa propre route, après un nouveau contrôle d'accès.
    const serialise = JSON.stringify(correction);
    expect(serialise).not.toContain("abc123def");
  });

  it("annonce l'absence de correction et de vidéo sans se tromper de champ", async () => {
    findFirstExercice.mockResolvedValue({ correction_texte: null, correction_video_ref: null });
    const correction = await obtenirCorrectionExercice(BigInt(1), BigInt(9));
    expect(correction?.texte).toBeNull();
    expect(correction?.videoDisponible).toBe(false);
  });

  it("rend null quand l'exercice n'est pas visible", async () => {
    findFirstExercice.mockResolvedValue(null);
    expect(await obtenirCorrectionExercice(BigInt(1), BigInt(9))).toBeNull();
  });
});
